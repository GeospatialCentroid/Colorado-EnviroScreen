...
generateDataForShiny <- function(removeNativeLand, removeZeroPop, version, spanish = FALSE){
  ###
  ...
  acsData <- list.files(path = "data/acs", pattern = ".csv", full.names = TRUE)%>%
    purrr::map(read_csv)%>%
    purrr::reduce(rbind)%>%
  ...
  c_data <- read_csv(paste0("data/envScreenScores/county_",version,"_geomZero.csv"))%>%
    dplyr::left_join(county, by = "GEOID")
  ct_data <- read_csv(paste0("data/envScreenScores/censusTract_",version,"_geomZero.csv"))%>%
    dplyr::left_join(censusTract, by = "GEOID")
  cbg_data <- read_csv(paste0("data/envScreenScores/censusBlockGroup_",version,"_geomZero.csv"))%>%
  ...
  if(isFALSE(spanish)){
    ...
    df <- df %>%
      dplyr::select(
        ...
        ,"Pollution and Climate Burden Percentile"= "pollClimBurden_Pctl"
        ,"Pollution and Climate Burden Scale Value"= "scaledpollClimate"
        ,"Health and Social Factors Score"="popCharacteristic"
        ,"Health and Social Factors Percentile"="popCharacteristic_Pctl"
        ...
        ,"Sensitive Populations Percentile"="senPop_Pctl"
  ...
  df <- df %>%
    dplyr::select(
      ...
      ,"Proximidad y volumen de tránsito"="trafficeProx"
      ,"Percentil de proximidad y volumen de tránsito"="trafficeProx_pcntl"
      ...
      ,"Proximidad a ubicaciones de minería" = "mining"
      ,"Percentil de proximidad a ubicaciones de minería" = "mining_pcntl"
      ...
      ,"Llanuras aluviales"="floodplainPercent"
      ,"Percentil de llanuras aluviales"="floodplainPercent_pcntl"
      ,"Sequía"="drought"
      ...
      ,"Cardiopatías en adultos"="heartDisease"
      ,"Percentil de cardiopatías en adultos"="heartDisease_pcntl"
      ...
      ,"Predominio de cáncer"="cancer"
      ,"Percentil de predominio de cáncer"="cancer_pcntl"
      ,"Predominio de diabetes"="diabetes"
      ,"Percentil de predominio de diabetes"="diabetes_pcntl"
  ...
  # rdata delete_dsn 
  saveRDS(df, file = paste0("data/envScreenScores/allScores_",version,"_spanish.rds"))
  }
}
