...
getDrinkingWater <- function(geometry){
  # EnviroScreen drinking water indicator ----
  
  ## About ----
  
  # This indicator utilizes health-based violations to summarise drinking water
  # quality at the county level throughout the state. 
  # 
  # Specifications:
  #   - Community public water systems (PWS)
  #   - Not water haulers
  #   - Status = A
  #   - Years: 2010-2020
  #   - Resolved and unresolved violations of the following types: 
  #     + Maximum Contaminant Limit (MCL) exceedences for all analytes
  #     + Total Coliform Rule violations (acute and monthly)
  #     + Maximum residual disinfectant level violations
  #     + Treatment Technique violations 
  #     + Lead 90th Percentile exceedances (>15)
  # 
  # The duration of each violation is the primary unit of measure. For example, 
  # did a violation last one month, one year, or five years, etc.
  # 
  # Violations are summarised based on principle county served by calculating a 
  # population-weighted average of the total violation duration. 
  

  ### Import and prepare data ----
  
  #### County boundaries ----
  county <- setSpatialData(dataFolder = "data/", scale = "county")
  ...
  allPWS <- read.csv("data/drinkingWater/AllPWS_CommunityNTNC.csv", stringsAsFactors = F) %>%
  ...
  v <- read.csv("data/drinkingWater/Violations2010_2020.csv", stringsAsFactors = F)
  ...
  Pb <- read.csv("data/drinkingWater/Lead90th.csv", stringsAsFactors = F) # all Pb results
